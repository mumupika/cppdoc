...
function parseText(text: string): LineData[] {
    return text.split('\n').map(line => {
        const parts = line.trimEnd().replace(/[^\p{L}\p{N}]/gu, " ").split(/\s+/).filter(p => p.length > 0);
...
export function visualizeTextDiff(textA: string, textB: string) {
    ...
    for(const lineRowA of linesA) {
        for(const lineRowB of linesB) {
            if (lineRowA.tokens.length === 0 || lineRowB.tokens.length === 0) continue;
    ...
    const lineRenderMap = new Map<number, { yValues: { start: number, height: number }, wrapA: string[][], wrapB: string[][] }>();
    ...
    linesToRender.forEach(lineIdx => {
        ...
        if (greenWeight > 0) {
            ...
            ctx.fillRect(config.width / 2, currentY, config.width / 2, yValues.height);
        }
        ...
        if (linesB[lineIdx]) drawColumn(linesB[lineIdx].tokens, wrapB, config.width / 2, false);
